...
async function onRequestPut({ request, env, params }) {
  ...
  if (sets.length === 0) return json3({ ok: false, error: "No fields to update." }, 400);
...
async function onRequestGet3({ env }) {
  try {
    ...
    return json5({ ok: true, collections: res?.results || [] });
  } catch (err) {
    console.error("COLLECTIONS LIST ERROR:", err);
    return json5({ ok: false, error: "Server error." }, 500);
  }
}
__name(onRequestGet3, "onRequestGet");
async function onRequestPost4({ request, env }) {
  const auth = await requireAdmin4(request, env);
  if (!auth.ok) return json5({ ok: false, error: auth.error }, auth.status);
  let payload = {};
  try {
    payload = await request.json();
  } catch {
    return json5({ ok: false, error: "Invalid JSON body." }, 400);
  }
  ...
  try {
    ...
    return json5({ ok: true, collection: row }, 201);
  } catch (err) {
    console.error("COLLECTION CREATE ERROR:", err);
    const msg = String(err?.message || "").toLowerCase();
    if (msg.includes("unique") || msg.includes("constraint")) {
      return json5({ ok: false, error: "A collection with that Id/Slug already exists." }, 409);
    }
...
// ../.wrangler/tmp/pages-HPfR8d/functionsRoutes-0.8028873714068199.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-FIySvt/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-FIySvt/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7766445223278147.mjs.map
