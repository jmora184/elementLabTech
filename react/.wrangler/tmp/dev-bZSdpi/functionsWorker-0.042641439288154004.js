...
// .wrangler/tmp/pages-zDensV/functionsWorker-0.042641439288154004.mjs
...
async function onRequestPost10({ request, env }) {
  try {
    ...
    if (!secretKey) {
      return json16({ ok: false, error: "Missing STRIPE_SECRET_KEY on the server." }, 500);
    }
    if (!webhookSecret) {
      return json16({ ok: false, error: "Missing STRIPE_WEBHOOK_SECRET on the server." }, 500);
    }
    const signature = request.headers.get("stripe-signature");
    if (!signature) {
      return json16({ ok: false, error: "Missing Stripe-Signature header." }, 400);
    }
    ...
    try {
      ...
      return json16({ ok: false, error: `Webhook signature verification failed: ${String(err?.message || err)}` }, 400);
    }
    if (event.type !== "checkout.session.completed" && event.type !== "checkout.session.async_payment_succeeded" && event.type !== "checkout.session.async_payment_failed") {
      return json16({ ok: true, ignored: true, eventType: event.type });
    }
    const sessionFromEvent = event.data.object || {};
    const stripeSessionId = String(sessionFromEvent?.id || "").trim();
    if (!stripeSessionId) {
      return json16({ ok: false, error: "Missing Checkout Session ID on the webhook event." }, 400);
    }
    ...
    if (!userId || !Number.isFinite(totalAmount) || totalAmount <= 0) {
      return json16({ ok: false, error: "Missing required checkout session fields." }, 400);
    }
    ...
    if (duplicateLookupValue) {
      ...
      if (existing?.id) {
        ...
        return json16({ ok: true, duplicate: true, purchaseId: existing.id, eventType: event.type });
      }
    }
    const insertColumns = [];
    const placeholders = [];
    ...
    if (!insertColumns.length) {
      return json16({ ok: false, error: "Purchases table is missing required columns." }, 500);
    }
    ...
    return json16({ ok: false, error: String(err?.message || "Server error processing Stripe webhook.") }, 500);
  }
}
...
async function onRequestGet6({ request, env }) {
  try {
    ...
    const user = await getUserFromSession(env, token);
    if (!user) {
      return json17({ ok: false, error: "Unauthorized" }, 401);
    }
...
// .wrangler/tmp/bundle-i74wsA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-i74wsA/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.042641439288154004.js.map
