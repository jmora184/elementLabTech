...
async function onRequestGet4({ env }) {
  try {
    ...
    return json11({ ok: true, collections: res?.results || [] });
  } catch (err) {
...
async function onRequestPost6({ request, env }) {
  ...
  if (!name) return json11({ ok: false, error: "Name is required." }, 400);
  ...
  try {
    ...
    return json11({ ok: true, collection: row }, 201);
  } catch (err) {
    console.error("COLLECTION CREATE ERROR:", err);
    const msg = String(err?.message || "").toLowerCase();
    if (msg.includes("unique") || msg.includes("constraint")) {
      return json11({ ok: false, error: "A collection with that Id/Slug already exists." }, 409);
    }
...
async function onRequestPost7({ request, env }) {
  try {
    ...
    if (!firstName || !lastName || !email || !message) {
      return json12({ ok: false, error: "Please complete all fields." }, 400);
    }
    if (!email.includes("@")) {
      return json12({ ok: false, error: "Please provide a valid email." }, 400);
    }
    ...
    if (!resendApiKey) {
      return json12({ ok: false, error: "Server is not configured for email yet (missing RESEND_API_KEY)." }, 500);
    }
    ...
    if (!resendRes.ok) {
      ...
      return json12({ ok: false, error: "Failed to send email." }, 502);
    }
    return json12({ ok: true, id: resendData?.id || null });
  } catch (err) {
...
// ../.wrangler/tmp/pages-TOfjIu/functionsRoutes-0.014493665146702472.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-ceafQc/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-ceafQc/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.17724284018930025.mjs.map
