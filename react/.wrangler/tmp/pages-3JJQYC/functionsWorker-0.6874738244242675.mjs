...
async function onRequestPost7({ request, env }) {
  try {
    ...
    if (!firstName || !lastName || !email || !message) {
      return json10({ ok: false, error: "Please complete all fields." }, 400);
    }
    if (!email.includes("@")) {
      return json10({ ok: false, error: "Please provide a valid email." }, 400);
    }
    ...
    if (!resendApiKey) {
      return json10({ ok: false, error: "Server is not configured for email yet (missing RESEND_API_KEY)." }, 500);
    }
    ...
    if (!resendRes.ok) {
      ...
      return json10({ ok: false, error: "Failed to send email." }, 502);
    }
    return json10({ ok: true, id: resendData?.id || null });
  } catch (err) {
    console.error("CONTACT ROUTE ERROR:", err);
    return json10({ ok: false, error: "Server error." }, 500);
  }
}
__name(onRequestPost7, "onRequestPost");

// ../.wrangler/tmp/pages-3JJQYC/functionsRoutes-0.06210541876018261.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-1ADzrP/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-1ADzrP/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6874738244242675.mjs.map
