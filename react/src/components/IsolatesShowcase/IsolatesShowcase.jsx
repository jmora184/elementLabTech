...
const MOCK_ISOLATE_PRODUCTS = [
  {
    id: "isolate-alpha-limonene",
    name: "Alpha Limonene Isolate",
    tagline: "Citrus-forward • 98%+ purity",
    imageSrc: vaporImg,
    productPath: "/product/fruity-fusion-forward",
  },
  {
    id: "isolate-beta-caryophyllene",
    name: "Beta Caryophyllene Isolate",
    tagline: "Warm spice • Structured depth",
    imageSrc: infusedFlowerImg,
    productPath: "/product/matrix-collection",
  },
  {
    id: "isolate-linalool",
    name: "Linalool Isolate",
    tagline: "Floral lift • Smooth finish",
    imageSrc: beverageImg,
    productPath: "/product/emerald-cut",
  },
  {
    id: "isolate-myrcene",
    name: "Myrcene Isolate",
    tagline: "Rounded body • Lasting aroma",
    imageSrc: gummiesImg,
          ...
          <section className="ts-section">
            <div className="ts-inner">
              <div className="ts-desktopCardStack">
                {MOCK_ISOLATE_PRODUCTS.map((product, index) => (
                  <div key={`${product.id}-${index}`} id={`card-${product.id}`}>
                    <IsolateCard product={product} isMobile={false} />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="ts-mobileCardStack" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              {MOCK_ISOLATE_PRODUCTS.map((product, index) => (
                <div key={`${product.id}-${index}`} id={`card-${product.id}`} style={{ width: "100%" }}>
                  <IsolateCard product={product} isMobile={true} />
                </div>
              ))}
            </div>
          </div>
...
function IsolateCard({ product, isMobile }) {
