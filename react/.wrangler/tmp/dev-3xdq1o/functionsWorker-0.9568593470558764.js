...
// .wrangler/tmp/pages-J9Xq3c/functionsWorker-0.9568593470558764.mjs
...
function json11(body, status = 200, extraHeaders = {}) {
  return new Response(JSON.stringify(body), {
    status,
...
async function onRequestGet4({ env, params }) {
  const id = params?.id;
  if (!id) return json11({ ok: false, error: "Missing collection id." }, 400);
  try {
    const row = await env.DB.prepare(
    ...
    if (!row) return json11({ ok: false, error: "Collection not found." }, 404);
    return json11({ ok: true, collection: row });
  } catch (err) {
    console.error("COLLECTION GET ERROR:", err);
    return json11({ ok: false, error: "Server error." }, 500);
  }
}
...
async function onRequestPut({ request, env, params }) {
  ...
  if (sets.length === 0) return json11({ ok: false, error: "No fields to update." }, 400);
...
async function onRequest4(context) {
  ...
  if (!slug) return json12({ ok: false, error: "Missing slug" }, 400);
  if (request.method === "GET") {
    try {
      const profile = await env.DB.prepare(
        "SELECT * FROM flavor_profiles WHERE slug=? LIMIT 1"
      ).bind(slug).first();
      if (!profile) return json12({ ok: false, error: "Not found" }, 404);
      let images = [];
      let documents = [];
      try {
      ...
      return json12({ ok: true, profile: normalized, images, documents });
    } catch (err) {
      console.error("PROFILE GET ERROR:", err);
      return json12({ ok: false, error: String(err?.message || err) }, 500);
    }
  }
  if (request.method === "PUT") {
    const gate = await requireAdmin9(request, env);
    if (!gate.ok) return json12({ ok: false, error: gate.error }, gate.status);
    let body = null;
    try {
      body = await request.json();
    } catch {
      return json12({ ok: false, error: "Invalid JSON" }, 400);
    }
    try {
      const existing = await env.DB.prepare(
        "SELECT * FROM flavor_profiles WHERE slug=? LIMIT 1"
      ).bind(slug).first();
      if (!existing) return json12({ ok: false, error: "Not found" }, 404);
      ...
      return json12({ ok: true, profile: normalized });
    } catch (err) {
      console.error("PROFILE PUT ERROR:", err);
      return json12({ ok: false, error: String(err?.message || err) }, 500);
    }
  }
  return json12({ ok: false, error: "Method not allowed" }, 405);
}
...
async function onRequestGet5({ request, env }) {
  const auth = await requireAdmin10(request, env);
  if (!auth.ok) return json13({ ok: false, error: auth.error }, auth.status);
  try {
    const res = await env.DB.prepare(
      `SELECT
         p.id,
    ...
    return json13({ ok: true, orders: res?.results || [] });
  } catch (err) {
    console.error("ADMIN ORDERS LIST ERROR:", err);
    return json13({ ok: false, error: "Server error loading orders." }, 500);
  }
}
...
async function onRequestGet6({ request, env }) {
  try {
    ...
    if (!sessionId) {
      return json14({ error: "Missing session_id." }, 400);
    }
    ...
    return json14({ error: String(err?.message || "Server error retrieving checkout session.") }, 500);
  }
}
...
function json15(body, status = 200, extraHeaders = {}) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { "Content-Type": "application/json", ...CORS_HEADERS10, ...extraHeaders }
  });
}
...
async function onRequestGet7({ env }) {
  try {
    ...
    return json15({ ok: true, collections: res?.results || [] });
  } catch (err) {
    console.error("COLLECTIONS LIST ERROR:", err);
    return json15({ ok: false, error: "Server error." }, 500);
  }
}
...
async function onRequestPost6({ request, env }) {
  ...
  if (!name) return json15({ ok: false, error: "Name is required." }, 400);
  ...
  try {
    ...
    return json15({ ok: true, collection: row }, 201);
  } catch (err) {
    console.error("COLLECTION CREATE ERROR:", err);
    const msg = String(err?.message || "").toLowerCase();
    if (msg.includes("unique") || msg.includes("constraint")) {
      return json15({ ok: false, error: "A collection with that Id/Slug already exists." }, 409);
    }
...
async function onRequestPost7({ request, env }) {
  try {
    ...
    if (!firstName || !lastName || !email || !message) {
      return json16({ ok: false, error: "Please complete all fields." }, 400);
    }
    if (!email.includes("@")) {
      return json16({ ok: false, error: "Please provide a valid email." }, 400);
    }
    ...
    if (!resendApiKey) {
      return json16({ ok: false, error: "Server is not configured for email yet (missing RESEND_API_KEY)." }, 500);
    }
    ...
    if (!resendRes.ok) {
      ...
      return json16({ ok: false, error: "Failed to send email." }, 502);
    }
    return json16({ ok: true, id: resendData?.id || null });
  } catch (err) {
...
async function onRequestPost8({ request, env }) {
  try {
    const secretKey = String(env?.STRIPE_SECRET_KEY || "").trim();
    if (!secretKey) {
      return json17({ error: "Missing STRIPE_SECRET_KEY on the server." }, 500);
    }
    const cookieHeader = request.headers.get("Cookie") || "";
    const cookies = parseCookie(cookieHeader);
    const token = cookies?.el_session || "";
    const user = await getUserFromSession(env, token);
    if (!user) {
      return json17({ error: "Not authenticated." }, 401);
    }
    const body = await request.json().catch(() => ({}));
    const items = sanitizeItems(body?.items);
    if (!items.length) {
      return json17({ error: "Your cart is empty or contains invalid pricing." }, 400);
    }
    ...
    return json17({ sessionId: session.id, url: session.url || null });
  } catch (err) {
    console.error("CREATE CHECKOUT SESSION ERROR:", err);
    return json17({ error: String(err?.message || "Server error creating checkout session.") }, 500);
  }
}
...
async function onRequestPost10({ request, env }) {
  try {
    ...
    if (!secretKey) {
      return json19({ ok: false, error: "Missing STRIPE_SECRET_KEY on the server." }, 500);
    }
    if (!webhookSecret) {
      return json19({ ok: false, error: "Missing STRIPE_WEBHOOK_SECRET on the server." }, 500);
    }
    const signature = request.headers.get("stripe-signature");
    if (!signature) {
      return json19({ ok: false, error: "Missing Stripe-Signature header." }, 400);
    }
    ...
    try {
      ...
      return json19({ ok: false, error: `Webhook signature verification failed: ${String(err?.message || err)}` }, 400);
    }
    if (event.type !== "checkout.session.completed" && event.type !== "checkout.session.async_payment_succeeded" && event.type !== "checkout.session.async_payment_failed") {
      return json19({ ok: true, ignored: true, eventType: event.type });
    }
    const sessionFromEvent = event.data.object || {};
    const stripeSessionId = String(sessionFromEvent?.id || "").trim();
    if (!stripeSessionId) {
      return json19({ ok: false, error: "Missing Checkout Session ID on the webhook event." }, 400);
    }
    ...
    if (!userId || !Number.isFinite(totalAmount) || totalAmount <= 0) {
      return json19({ ok: false, error: "Missing required checkout session fields." }, 400);
    }
    ...
    if (duplicateLookupValue) {
      ...
      if (existing?.id) {
        ...
        return json19({ ok: true, duplicate: true, purchaseId: existing.id, eventType: event.type });
      }
    }
    const insertColumns = [];
    const placeholders = [];
    ...
    if (!insertColumns.length) {
      return json19({ ok: false, error: "Purchases table is missing required columns." }, 500);
    }
    const result = await env.DB.prepare(
    ...
    return json19({ ok: false, error: String(err?.message || "Server error processing Stripe webhook.") }, 500);
  }
}
...
async function onRequestGet8({ request, env }) {
  try {
    ...
    const user = await getUserFromSession(env, token);
    if (!user) {
      return json20({ ok: false, error: "Unauthorized" }, 401);
    }
    ...
    return json20({ ok: false, error: "Server error." }, 500);
  }
}
...
// .wrangler/tmp/bundle-3w61FE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3w61FE/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9568593470558764.js.map
