...
function pickCarrierCollection(collections = []) {
  const rows = Array.isArray(collections) ? collections : [];
  if (!rows.length) return null;

  const preferredIds = ["carrier-collection", "carriers-collection", "carriers"];
  ...
  const byId = rows.find((row) => /carrier/i.test(String(row?.id || "")));
  if (byId) return byId;

  const byName = rows.find((row) => /carrier/i.test(String(row?.name || "")));
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const selectedCollection = pickCarrierCollection(list?.collections || []);
        if (!selectedCollection?.id) {
          throw new Error("No carrier collection found. Create a dedicated collection with id 'carrier-collection' (or a name/id containing 'carrier').");
        }
        ...
        if (!alive) return;
        setError(err?.message || "Failed to load carrier collection.");
      } finally {
    ...
    <>
      ...
      <div className="pp-page">
        <main className="pp-container">
          <div className="pp-topGrid">
            <section className="pp-card pp-galleryCard" aria-label="Carrier gallery">
              ...
              <div className="pp-galleryMeta">
                <span className="pp-badge">{collection?.badge || "Carriers"}</span>
                <div className="pp-collectionName">{collection?.name || "Carriers Collection"}</div>
                <p className="pp-muted" style={{ marginTop: 8 }}>{collection?.tagline || "Carrier bases tuned for clean formulation and stable blends."}</p>
              </div>
            </section>

            <section className="pp-card pp-infoCard" aria-label="Carrier details">
              ...
              <div className="pp-tabPanel">
                {tab === "Details" && (
                  <div className="pp-flavorInfo">
                    <div className="pp-flavorTableWrap" aria-label="Carrier flavor table">
            ...
            <section className="pp-card pp-buyCard" aria-label="Carrier actions">
              <div className="pp-buySticky">
                ...
                <button
                  ...
                  onClick={() => {
                    addCartItem({
                      productId: String(collection?.id || "carrier-collection"),
                      collectionName: String(collection?.name || "Carriers"),
      ...
      {isAdmin && showEditCollection && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label="Edit carrier collection"
          onClick={(e) => {
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <h3 className="pp-h3" style={{ margin: 0 }}>Edit Carrier Collection</h3>
      ...
      {isAdmin && showAddProfile && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label="Add carrier profile"
          onClick={(e) => {
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <h3 className="pp-h3" style={{ margin: 0 }}>Add Carrier Profile</h3>
